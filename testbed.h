...
void testbed_add_metrics(struct sk_buff *skb, struct testbed_metrics *testbed)
{
	...
	if (ntohs(ethh->h_proto) == ETH_P_IP) {
		...
		qdelay = (ktime_get_ns() - skb_ts) >> 15;
	
		qdelay = int2fl((__force __u32)qdelay, QDELAY_M, QDELAY_E, &qdelay_remainder);
		if (qdelay_remainder > 20) {
			pr_info("High (>20) queue delay remainder:  %u\n", qdelay_remainder);
		}
#else /* old behaviour - no encoding */
		/* convert to ms */
		qdelay = ((__force __u64)(ktime_get_ns() - ktime_to_ns(skb_get_ktime(skb)))) >> 20;
		if (qdelay > 2047) {
